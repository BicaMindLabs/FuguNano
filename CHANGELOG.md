...
- **`fanout skills` 本机 skills 母目录 + 按需注入(progressive disclosure)**：`skills index` 扫 **3 源**——user `~/.claude/skills` + `.system` 元技能(含官方 `skill-creator`/`plugin-creator`/`skill-installer`) + plugin 市场(`marketplaces/`, `plugin:skill` id, 按 id 去重避开 cache 重复)——的 SKILL.md frontmatter(解析 inline + YAML 折叠 `>-`) → 一个紧凑 catalog(`id·source·type·path·desc`, functional/note 前缀分类);`skills list/match/show/inject`(均支持 `--source` 过滤);`dispatch --skills "a,b"` 把选中 skill 注入该 agent context——弱模型只爬它该爬的,不被 500+ skill 淹没。三步:index 母目录 → Planner 读目录分配 → dispatch --skills 各注入。本机实测 556 skill(user 364 / plugin 187 / system 5)。**`skills forge` 闭环 沉淀→创建→放回分类**:取料(`--from-experience <ws/slug>` / `--source` / stdin)→ 候选门(料够厚才沉淀)→ 组装 authoring brief →(`--agent` 则派 worker 注入 `skill-creator` 去写, 否则打印 brief)→ 加 **`skills validate`** 验收门(镜像官方 `quick_validate.py` 的检查:SKILL.md/frontmatter/name hyphen-case≤64/description≤1024 无尖括号/合法 key;自包含无 PyYAML 依赖,`--official` 本机有 quick_validate.py 时优先用官方;forge 闭环=写完→validate 过→index --refresh)→ 回灌母目录。**authoring 委托官方 skill-creator, 不重造蒸馏**(skills-master/Skill Forge 是同域)。+44 自测(skills 51 含双向闭环 + dispatch 2)。子命令 17→18,测试套 17→18,总断言→312。
