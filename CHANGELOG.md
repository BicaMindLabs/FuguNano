...
- **Engineering refactor — shared lib layer + caching (4 Codex-reviewed iterations)**: extracted `fuguectl-lib.sh` (`die`/`warn`/`say`, status symbols, portable `fx_mtime`, an atomic TTL cache layer `fcache_get`/`put`/`clear`, and `fx_cache_root`) and `fuguectl-testlib.sh` (`pass`/`fail` counters + `ok`/`skip`/`tdone`) — every tool/suite now sources them instead of copy-pasting (−16 duplicate `die()`, −18 test-boilerplate copies, −5 inline `CACHE_ROOT`). `preflight --probe` now caches endpoint liveness through the TTL cache (the HTTP code only, never the key; `FUGUE_PROBE_TTL`, default 20s); diagnostics (`doctor`/`fleet status`) stay deliberately un-cached so they read fresh. selftest total assertions 312 -> 313.
