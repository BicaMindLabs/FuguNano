#!/usr/bin/env bash
# fuguectl-loop.sh — Phase 5 review-fix loop state machine (turns SKILL.md pseudocode into runnable+testable)
#
...
#
# State layout (${FUGUE_CACHE:-<repo>/.fuguectl-cache}/loop/):
...
need_init(){ [ -f "$META" ] || die "loop not init (run fuguectl loop init first)"; }
...
cmd_status(){
  ...
  echo "── fuguectl loop ── max=$max  best n=$bn sha=${bsha:-—}  task=${task:-—}"
  if [ -s "$ROUNDS" ]; then
