...
| **Backend Implementers** (auto parallel dispatch) | `cc-claude` (Claude, in `$FUGUE_CC_CLAUDE`) + the Chinese-model clones `cc-deepseek` `cc-glm` `cc-kimi` `cc-minimax` `cc-mimo` `cc-stepfun` `cc-doubao` `cc-ark-auto` (in `$FUGUE_CC_WORK`) | `"$FO" dispatch cc-X --harness fugue-cc ...`                                                                                                                                         |
...
**① Open the round cache** — declare the N tasks this round dispatches (the parallel dispatch manifest), so the join barrier can later require all N back. `CACHE=orchestration/fuguectl/fuguectl-cache.sh`:
