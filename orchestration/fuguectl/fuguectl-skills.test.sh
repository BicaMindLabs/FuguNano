...
# skip plugin source
out="$(FUGUE_SKILLS_NO_PLUGINS=1 bash "$S" index --refresh)"
...
bash "$HERE/fuguectl-experience.sh" add code "distilled method" --from "$MAT" >/dev/null 2>&1
...
FUGUE_CC_BIN="$FUGUE_CC_STUB" bash "$S" forge --name viaworker --source "$MAT" --agent cc-x >/dev/null 2>&1
...
ok "closed loop: validated skill enters mother dir after re-index" 'o=$(bash "$S" list --type functional); [[ "$o" == *"forged-skill"* ]]'
