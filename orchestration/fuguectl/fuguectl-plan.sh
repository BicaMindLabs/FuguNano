#!/usr/bin/env bash
# fuguectl-plan.sh — multi-model planning panel: send "decompose goal" to N planning models at once, each Writes its plan,
...
#   env: FUGUE_CC_BIN(stub for tests) / FUGUE_CACHE
set -uo pipefail
...
for m in "${MS[@]}"; do
  ...
  bash "$HERE/fuguectl-dispatch.sh" "$m" --template plan \
