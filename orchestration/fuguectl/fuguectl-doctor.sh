#!/usr/bin/env bash
# fuguectl-doctor.sh — environment recon + workflow advisor
#
...
say "╔══════════════════════════════════════════════╗"
say "║  parallel dispatch workflow — environment recon + advisor ║"
say "╚══════════════════════════════════════════════╝"
...
for spec in "claude:Planner/Executor (Claude Code)" "codex:Reviewer (independent frontier)" \
            "fugue-cc:Provider runtime bridge for the implementer fleet" "agy:Frontend (Antigravity)" \
            "opencode:alternate implementer/reviewer" "node:dep" "git:dep" "tmux:dep (provider panes)"; do
  c="${spec%%:*}"; desc="${spec#*:}"
...
say ""; say "── summary ──"
ncli=0; for c in claude codex fugue-cc agy opencode; do [ "${ROLE[$c]:-0}" = 1 ] && ncli=$((ncli+1)); done
...
if [ "${ROLE[fugue-cc]:-0}" = 1 ] && [ "$impl_ready" -ge 2 ] && [ "${ROLE[codex]:-0}" = 1 ]; then
  add "✅ full fleet workflow: fugue-cc fleet → $impl_ready backends implement in parallel(each its own worktree) → Codex reviews → Phase 5 bounded loop. Results go through fuguectl-cache + join barrier(send N receive N)."
elif [ "$impl_ready" -ge 1 ] && [ "${ROLE[fugue-cc]:-0}" = 0 ]; then
  add "⚙️ single-machine lite: no fugue-cc provider → dispatch sequentially via the /cn:* plugin(no automatic review loop). Install the provider backend to unlock the full fleet workflow."
elif [ "$impl_ready" -ge 1 ]; then
  add "⚙️ half setup: $impl_ready backend(s) available, dispatch manually as needed."
else
...
if [ "$QUIET" -eq 1 ]; then
  echo "agents=$ncli backends_ready=$impl_ready/9 fugue-cc=${ROLE[fugue-cc]:-0} codex=${ROLE[codex]:-0} agy=${ROLE[agy]:-0}"
fi
