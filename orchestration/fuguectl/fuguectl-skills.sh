#!/usr/bin/env bash
# fuguectl-skills.sh — local skills master catalog (mother dir) + on-demand injection to agents (progressive disclosure)
#
# fuguectl philosophy = only feed what's needed. Hundreds of local skills, can't stuff all into every (weak) agent. Flow:
...
#   env: FUGUE_SKILLS_ROOT(default ~/.claude/skills) FUGUE_PLUGINS_ROOT(default ~/.claude/plugins/marketplaces)
#        FUGUE_SKILLS_CATALOG(catalog path) FUGUE_SKILLS_NOTE_RE(note prefix regex) FUGUE_SKILLS_NO_PLUGINS=1(skip plugin)
set -uo pipefail
# shellcheck source=/dev/null
. "$(dirname "${BASH_SOURCE[0]}")/fuguectl-lib.sh"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="${FUGUE_SKILLS_ROOT:-$HOME/.claude/skills}"
PLUGINS="${FUGUE_PLUGINS_ROOT:-$HOME/.claude/plugins/marketplaces}"
CATALOG="${FUGUE_SKILLS_CATALOG:-${FUGUE_STATE:-$HOME/.config/fugue}/skills-catalog.tsv}"
NOTE_RE="${FUGUE_SKILLS_NOTE_RE:-^(wdkns|book|csdiy|dlai|mit|mooc|child|tu-online)}"
...
_scan(){
  {
    ...
    if [ "${FUGUE_SKILLS_NO_PLUGINS:-0}" != 1 ] && [ -d "$PLUGINS" ]; then
...
cmd_forge(){
  local name="" fromexp="" source="" from_stdin=0 agent="" harness="fugue-cc" targetdir="$ROOT" minchars=200
  while [ "$#" -gt 0 ]; do case "$1" in
    --name)            name="${2:-}"; shift 2;;
  ...
  if [ -n "$fromexp" ]; then
    ...
    material="$(bash "$HERE/fuguectl-experience.sh" show "$ws" "$slug" 2>/dev/null | sed '1,/^---$/d; /^---$/d')"
  ...
  if [ -n "$agent" ]; then
    ...
    bash "$HERE/fuguectl-dispatch.sh" "$agent" --harness "$harness" --skills skill-creator --prompt-file "$brief"; local rc=$?
    rm -f "$brief"
    echo "→ after worker finishes run acceptance gate + reabsorb: \`fuguectl skills validate $name && fuguectl skills index --refresh\`"
    return "$rc"
  fi
  echo "── forge brief (name=$name - target=$target) — hand to worker / skill-creator to execute ──"
  cat "$brief"; rm -f "$brief"
  echo ""
  echo "→ after skill is written pass acceptance gate then reabsorb into mother dir (closed loop): \`fuguectl skills validate $name && fuguectl skills index --refresh\`"
}
