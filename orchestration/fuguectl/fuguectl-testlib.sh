#!/usr/bin/env bash
# fuguectl-testlib.sh — shared harness for the fuguectl *.test.sh suites.
#
# SOURCED near the top of each suite (after HERE is set):
#     # shellcheck source=/dev/null
#     . "$(dirname "${BASH_SOURCE[0]}")/fuguectl-testlib.sh"
#
...
# suite name for the summary line (e.g. fuguectl-dispatch), from the sourcing file.
T_PROG="${BASH_SOURCE[1]:-${0:-fuguectl-test}}"; T_PROG="${T_PROG##*/}"; T_PROG="${T_PROG%.test.sh}"
