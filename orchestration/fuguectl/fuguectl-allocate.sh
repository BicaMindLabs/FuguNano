#!/usr/bin/env bash
# fuguectl-allocate.sh — task type → recommended model (bench prior + real-world experience Bayesian mix)
#
...
#   env: FUGUE_ALLOCATION(bench table) FUGUE_ALLOCATION_STATS(stats file) FUGUE_ALLOCATION_LEDGER(ledger)
#        FUGUE_ALLOCATE_KAPPA(prior strength, default 4) FUGUE_ALLOCATE_SEED(TS sampling seed, for tests) FUGUE_STATE
set -uo pipefail
...
_score_rows(){
  ...
  awk -F'\t' -v task="$task" -v blist="$models" -v kappa="$KAPPA" -v up="$UNLISTED_PRIOR" \
      -v sample="$sample" -v seed="${FUGUE_ALLOCATE_SEED:-}" '
...
cmd_rank(){
  ...
  if models="$(bench_list "$task")"; then :; else
    ...
    echo "fuguectl-allocate: unknown task type '$task' → falling back to fallback ($models)" >&2
    task="fallback"
  fi
...
cmd_record(){
  ...
  # normalize: fugue-cc agent name cc-doubao → bench table bare name doubao, else experience feeds into different key, flywheel doesn't close
  agent="${agent#cc-}"
  ...
  bench_list "$task" >/dev/null 2>&1 || \
    echo "fuguectl-allocate: ⚠ '$task' not in bench table (allocation.tsv) — allocate queries fall back to fallback, these records won't be read; to take effect add '$task' to the table" >&2
