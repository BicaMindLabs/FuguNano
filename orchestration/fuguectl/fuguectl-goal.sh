#!/usr/bin/env bash
# fuguectl-goal.sh — goal mode: declarative goal spec + deterministic acceptance gate
#
...
cmd_template(){
  cat <<'EOF'
# fuguectl goal spec — `fuguectl goal check <thisfile>` runs gate to decide if goal is met
outcome: <one-line goal>
