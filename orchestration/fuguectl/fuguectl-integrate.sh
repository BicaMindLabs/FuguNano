#!/usr/bin/env bash
# fuguectl-integrate.sh — Phase 3 integrate: cherry-pick each agent worktree's changes back to main branch
#
...
work=""; agents=""; ws_parent=".fugue-cc/workspaces"; onconflict="abort"; ownership=""; task=""; dry=0
while [ "$#" -gt 0 ]; do
  case "$1" in
    --work)       work="${2:-}"; shift 2;;
...
for ag in $agents; do
  ...
  git -C "$wt" add -A
  git -C "$wt" -c user.email=fugue-cc@local -c user.name="$ag" commit -q -m "$ag: $files" || {
  ...
  if git -C "$work" -c user.email=fugue-cc@local -c user.name=fuguectl-integrate cherry-pick "$sha" >/dev/null 2>&1; then
