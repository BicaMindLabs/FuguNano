#!/usr/bin/env bash
# fuguectl-workspace.sh — Workspace context isolation (inspired by Zleap-Agent)
...
resolve_models(){  # @bench:<type> → through allocation; otherwise verbatim
  case "$1" in
    @bench:*) bash "$HERE/fuguectl-allocate.sh" "${1#@bench:}" 2>/dev/null;;
    *) printf '%s' "$1";;
  esac
}
...
cmd_context(){
  ...
  local exp; exp="$(bash "$HERE/fuguectl-experience.sh" recall "$name" --limit 3 2>/dev/null || true)"
  [ -n "$exp" ] && { echo ""; printf '%s\n' "$exp"; }
  echo ""
  echo "### History"
