#!/usr/bin/env bash
# fuguectl-loop.test.sh — fuguectl-loop.sh state machine self-test (exit-state decision + keep-best)
set -uo pipefail
...
ok "init writes meta max=3" 'grep -q "max_rounds=3" "$FUGUE_CACHE/loop/meta"'
ok "init best_n=-1 (unset)" 'grep -q "best_n=-1" "$FUGUE_CACHE/loop/meta"'
...
ok "first record updates best_n=3" 'grep -q "best_n=3" "$FUGUE_CACHE/loop/meta"'
ok "first record updates best_sha=sha1" 'grep -q "best_sha=sha1" "$FUGUE_CACHE/loop/meta"'
...
ok "round2 findings decreased → best updates n=2" 'grep -q "best_n=2" "$FUGUE_CACHE/loop/meta"'
...
ok "no improvement → best_sha stays sha2" 'grep -q "best_sha=sha2" "$FUGUE_CACHE/loop/meta"'
...
ok "verdict 'needs fix' normalized to NEEDSFIX" '[ "$(cut -f3 "$FUGUE_CACHE/loop/rounds.tsv" | tail -1)" = NEEDSFIX ]'
