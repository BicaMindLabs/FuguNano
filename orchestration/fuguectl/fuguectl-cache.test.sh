#!/usr/bin/env bash
# fuguectl-cache.test.sh — self-test for fuguectl-cache.sh (CI / local)
set -uo pipefail
...
ok "init writes manifest 3 lines" '[ "$(wc -l <"$FUGUE_CACHE/round-1/manifest.tsv")" -eq 3 ]'
...
ok "result lands in cache after put" '[ -f "$FUGUE_CACHE/round-1/t1.result" ]'
