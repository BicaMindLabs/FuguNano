#!/usr/bin/env bash
# fuguectl-allocate.test.sh — static bench backward compat + adaptive (bench prior + real-world posterior) mix
set -uo pipefail
...
ok "ledger feed clears ledger by default" '[ ! -s "$FUGUE_ALLOCATION_LEDGER" ]'
# --keep retains ledger
printf 'code\tcc-zeta\n' > "$FUGUE_ALLOCATION_LEDGER"
bash "$A" feed --from-ledger --result ok --keep >/dev/null
ok "feed --keep retains ledger" '[ -s "$FUGUE_ALLOCATION_LEDGER" ]'
...
o1="$(FUGUE_ALLOCATE_SEED=5 bash "$A" code --sample)"; o2="$(FUGUE_ALLOCATE_SEED=5 bash "$A" code --sample)"
...
distinct="$(for s in $(seq 1 20); do FUGUE_ALLOCATE_SEED=$s bash "$A" code --sample --top; done | sort -u | grep -c .)"
