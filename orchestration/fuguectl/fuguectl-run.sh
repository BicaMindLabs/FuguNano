#!/usr/bin/env bash
# fuguectl-run.sh — cross-phase run status facade (axi-inspired: structured, machine-parsable read-only view)
#
# fuguectl's TASK / cache(barrier) / loop state is normally scattered across places, kept in the operator's head.
# This tool introduces a lightweight 'current run' context (.fuguectl-cache/run.meta records active TASK + round),
# aggregating cross-phase state into **one JSON object** —— so a parallel dispatch run can be structurally queried/resumed/driven
# (borrowing no-mistakes' axi idea, but without changing fuguectl's 'operator is orchestrator' model, just a read-only facade).
#
...
CACHE_SH="$HERE/fuguectl-cache.sh"
LOOP_SH="$HERE/fuguectl-loop.sh"
...
cmd_round(){
  ...
  [ -f "$RUN" ] || die "no active run (first fuguectl run set --task ...)"
  local t; t="$(rget task)"
  { printf 'task=%s\n' "$t"; printf 'round=%s\n' "$n"; } > "$RUN"
  echo "✓ round → $n"
}
...
_gather(){
  ...
  # cache(barrier) state — parse stable output of fuguectl-cache status
  ...
  if [ "$C_INIT" = true ] && [ "${C_PEND:-0}" -gt 0 ] 2>/dev/null; then
    ...
    NEXT="loop: ${L_DEC//\"/} — see fuguectl loop decide"
...
cmd_status(){
  ...
  _gather || die "no active run (first fuguectl run set --task ...)"
  if [ "$human" -eq 1 ]; then
...
cmd_next(){ _gather || die "no active run (first fuguectl run set --task ...)"; echo "$NEXT"; }
