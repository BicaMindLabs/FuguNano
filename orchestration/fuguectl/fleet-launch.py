...
usage: fleet-launch.py <project-dir> <cmd> [args...]
  ...
  (caller returns immediately, fugue-cc keeps running in the background).

Why this way:
  - strip CLAUDE_CODE_*: parent session's OAuth/session env leaks to child cc-* → fake 401.
  - pty.fork: fugue-cc needs a tty before it will start the agent pane.
  - fork + setsid: detach from the calling shell's session, fugue-cc survives after the caller exits.
  - drain pty: prevent fugue-cc from blocking when the output buffer fills up.
"""
import os
import pty
import sys
