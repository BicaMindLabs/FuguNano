...
| `orchestration/fanout/` | `fanout` 命令行（18 子命令）+ `SKILL.md`（5 阶段 + Phase 5 loop）+ `workspaces/` + `templates/` + 18 套测试 |
...
`orchestration/fanout/fanout` 是统一入口——一套 bash CLI,任意 agent(或你)都能驱动。`fanout help` 看全表;**18 个子命令**按在流水线里的位置分组:
...
| `fanout skills index\|list\|match\|show\|inject\|validate\|forge` | **Skills 母目录**——扫 **3 源**（user `~/.claude/skills` + `.system` 元技能含官方 `skill-creator` + plugin 市场, `plugin:skill` id）→ 一个紧凑 catalog（source · functional/note · path）；`inject` 只把选中 skill 喂给某 agent（配 `dispatch --skills` 做 progressive disclosure，弱模型只爬该爬的，不被 500+ 淹没）。**`forge`** 闭环：取沉淀的方法（`--from-experience`/`--source`）→ 候选门 → 派 worker 注入 `skill-creator` 写成新 skill → **`validate`** 质量门（镜像官方 `quick_validate.py`，`--official` 直接用它）→ `index --refresh` 回灌母目录（authoring 委托官方 skill-creator） |
...
| 命令 | 作用 |
|---|---|
| `fanout dispatch <target> [--harness ccb\|codex\|opencode] [--workspace ws] [--task-type T] [--skills a,b]` | 派活到**任意 harness**（ccb / codex / opencode）：渲 prompt → 跑 → 记日志；`--task-type` 喂路由飞轮，`--skills` 只注入需要的 skill |
