...
| `fanout allocate <type> [--top] [--sample]` · `record`·`feed`·`stats`·`reset`·`decay` | 任务类型→推荐模型——**bench 先验 + 实战后验**（Beta-Bernoulli）：冷启动=静态 bench 顺序，随 `record`/`feed` 喂 verdict 漂移。**`feed --from-ledger`** 闭合数据飞轮（`dispatch --task-type` 记 `(type,agent)`，整轮 verdict 后一把 `feed`）。**`--sample`** = Thompson Sampling（探索少样本 agent、不早锁赢家；Agrawal-Goyal 2012）；**`decay --gamma G`** 模型升级后折扣陈旧统计（Garivier-Moulines 2011） |
...
| `fanout dispatch <target> [--harness ccb\|codex\|opencode] [--workspace ws] [--template n] [--task-type T]` | 派活到**任意 harness**（ccb=Claude Code 分身 / codex / opencode）：渲 prompt → 跑 → 记日志；`--task-type` 喂分配飞轮 |
| `fanout cache init\|put\|fail\|barrier\|collect\|resume\|...` | 结果缓存 + **fan-in barrier**（发 N 收 N）+ 计时 + resume |
| `fanout integrate --work <repo> --agents "a b"` | **Phase 3**——各 worktree cherry-pick 到 `main`，**冲突隔离**：冲突的 agent 单独 `--abort` 并报告（保 `main` 干净），其余照常落地 |
...
| `fanout loop init\|record\|decide\|status` | **Phase 5** review-fix **状态机**——每轮 `record`（用 `--ask-user K` 给 finding 分类）→ `decide` 给退出态（DONE / CONFIRM / CONTINUE / **ASK_USER** / ESCALATE_MAX / ESCALATE_NONCONV）；keep-best 自动维护。`ASK_USER` = 机械 finding 自动修、碰意图的升级给人（借鉴 no-mistakes） |
| `fanout run set\|round\|status\|next\|clear` | **Run 状态门面**（axi-inspired）——轻量 "current run" 上下文,把跨阶段状态（active TASK / round / cache barrier N收M / loop decision / best）聚合成**一个机器可解析的 JSON** 对象,让一次 run 可查询/恢复,不改 operator 编排模型 |
| `fanout ccb-sync check\|adapt [--apply]` | ccb 更新后同步适配（版本漂移 · grafting 校验 · ccbd 重启） |
| `fanout selftest` | 跑全部 17 套测试（249 断言） |
...
make test        # cn-plugin + fanout selftest（249 断言）
make doctor      # 环境侦察
make help        # 所有目标
