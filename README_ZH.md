...
公开控制面是 [`fuguectl`](#fuguectl-命令行)，Claude Code skill 安装为 `/fugue`。
...
| 层                      | 状态                                                                          | 用途                                                |
| ----------------------- | ----------------------------------------------------------------------------- | --------------------------------------------------- |
| `orchestration/fuguectl/` | 生产可用的操作层：`fuguectl`、18 个子命令、18 套测试、322 个断言 | 日常多 agent 编码工作流                             |
...
| Implementers                 | 经 `fugue-cc` runtime 接入的 `cc-deepseek`、`cc-glm`、`cc-kimi`、`cc-qwen`、`cc-doubao`、`cc-minimax`、`cc-mimo`、`cc-stepfun`、`cc-longcat`，以及可选 `cc-claude` | 在隔离 worktree 里写代码             |
| Reviewer                     | Codex（`coder` / `codex`）                                                                                                                                                                             | 独立 ACCEPTED / NEEDS FIX verdict    |
| 可选前端 worker              | Antigravity（`agy`）                                                                                                                                                                                   | 只做 UI / frontend，不做 reviewer    |

## 快速开始

要求：macOS 或 Linux、Node >= 18.18、`git`、`tmux`，完整 fleet 需要配置好的 `fugue-cc` provider runtime，review 需要 Codex，前端可选 `agy`。
...
API key 不进仓。启动器读取 `~/.config/cc-model-secrets.env`；项目本地 provider 配置放在已 git-ignore 的 `.fugue-cc/` 下。
...
它会安装到 `~/.claude/skills/fugue`，并先备份旧副本。重启 Claude Code 后，可以用 `/fugue` 唤醒，或直接描述一个多 agent 任务。安装后的 skill 暴露 `fuguectl`：
...
`orchestration/fuguectl/fuguectl` 是主操作入口。精确语法看 `fuguectl help`。这 18 个子命令按工作流位置分组如下。
...
| `fuguectl fleet status\|up\|down` | 检查、启动或停止 `fugue-cc` worker fleet；启动前剥掉 `CLAUDE_CODE_*`，避免 OAuth 假 401。               |
| `fuguectl preflight [cfg]`        | Go/no-go gate：依赖、provider mount、provider config、no-Gemini policy、`.fugue-cc/` gitignore、可选 probe。 |
...
| 命令                                                                                                                    | 用途                                                                                                                 |
| ----------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `fuguectl dispatch <target> [--harness fugue-cc\|codex\|opencode] [--workspace ws] [--task-type T] [--skills a,b]` | 渲染或读取 prompt，可叠加 workspace 和 skill context，经 fugue-cc / Codex / OpenCode dispatch，并记录 routing 数据。 |
| `fuguectl cache init\|put\|fail\|barrier\|collect\|resume`                                                              | 持久结果缓存、join barrier、耗时和 resume 支持。                                                                   |
...
| `fuguectl runtime check\|adapt [--apply]`          | 检测并适配 provider / Claude Code 的版本漂移。                           |
| `fuguectl selftest`                                | 运行完整操作层测试：18 套测试、322 个断言。                                 |
...
Self-Harness 改进的是 harness 配置本身，而不是模型。fugue 的 loop 是对上海人工智能实验室 Self-Harness 论文（arXiv 2606.09498）的 engine-native 抽象。一次 run 做四件事：
...
| `orchestration/fugue-cc/`      | `fugue-cc` runtime 使用的脱敏 provider 配置模板。                                          |
...
- 真实 key 只放在 `~/.config/cc-model-secrets.env`，或项目本地、已 git-ignore 的 `.fugue-cc/`。
- 仓库只保留脱敏 examples。
